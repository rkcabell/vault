...
function assertContrast (
  ...
  assert.ok(
    ratio >= minRatio,
    `[${label}] contrast ${ratio.toFixed(
      2
    )} : 1 is below the ${minRatio} : 1 WCAG threshold`
  )
}
...
for (const theme of themes) {
  ...
  if (theme.card !== undefined && theme.cardForeground !== undefined) {
    test(`${name} — card-foreground on card`, () =>
      assertContrast(
        hsl(theme.cardForeground!),
        hsl(theme.card!),
        AA,
        `${name}/card-fg-on-card`
      ))
  }

  test(`${name} — primary-foreground on primary`, () =>
    assertContrast(
      hsl(theme.primaryForeground),
      hsl(theme.primary),
      AA,
      `${name}/primary`
    ))
  ...
  test(`${name} — tag-filter-include on background`, () =>
    assertContrast(
      theme.tagFilterInclude
        ? hex(theme.tagFilterInclude)
        : hex(theme.dark ? TAG_FILTER_INCLUDE_DARK : TAG_FILTER_INCLUDE_LIGHT),
      hsl(theme.background),
      AA,
      `${name}/tag-filter-include`
    ))

  test(`${name} — tag-filter-exclude on background`, () =>
    assertContrast(
      theme.tagFilterExclude
        ? hex(theme.tagFilterExclude)
        : hex(theme.dark ? TAG_FILTER_EXCLUDE_DARK : TAG_FILTER_EXCLUDE_LIGHT),
      hsl(theme.background),
      AA,
      `${name}/tag-filter-exclude`
    ))
