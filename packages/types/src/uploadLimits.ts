...
/**
 * One 2 GB cap for every kind, shared by the API and the web client. A lower
 * server cap would reject a file the client accepted — the file fails only
 * after being sent — which is why this lives here rather than in either app.
 * The value also matches the big-file storage path (Range streaming, Float
 * sizeBytes, 2 GiB thumbnail cap).
 *
 * Sent files only. A file found by a scan never touches this; indexCore has
 * no size gate at all.
 */
...
/** Which limit bucket a file falls into. MIME first, filename extension as fallback. */
